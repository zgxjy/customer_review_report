        ...
        if criteria_type == 'user_profile':
            ...
            for doc in cursor:
                for topic_item in doc.get("product_topic_result", []):
                    if (
                        topic_item.get("new_topic") == criteria_value
    ...
    def topic_polarity_stats(self, total_reviews):
        ...
        for doc in cursor:
            for topic_item in doc.get("product_topic_result", []):
                ...
                if stats[new_topic]["总数"] > 0:
                    stats[new_topic]["好评占比"] = stats[new_topic]["好评"] / stats[new_topic]["总数"]
                    stats[new_topic]["中差评占比"] = (stats[new_topic]["差评"] + stats[new_topic]["中评"]) / stats[new_topic]["总数"]
                    stats[new_topic]["提及率"] = stats[new_topic]["总数"] / total_reviews
...
class DataPipelineManager:
    ...
    def process_topics(self):
        ...
        for new_topic, count in top_good_topics:
            print("#" * 50)
            print(f"现在正在处理 new_topic：{new_topic}（好评数：{count}）")
            
            # 确保 new_topic 存在于结果字典中
            if new_topic not in topics_data:
                topics_data[new_topic] = topic_stats.get(new_topic, {})
            ...
            if summary_good:
                ...
                topics_data[new_topic]['好评摘要'] = summary_good
        
        # 处理中差评排名前 N 的话题
        print(f"\n正在处理中差评排名前{self.top_topics_count}的话题...")
        for new_topic, count in top_bad_topics:
            print("#" * 50)
            print(f"现在正在处理 new_topic：{new_topic}（中差评数：{count}）")
            ...
            if summary_bad:
                ...
                topics_data[new_topic]['中差评摘要'] = summary_bad
